...
The `mo_run` tool in configurable-agent currently spawns the `mo` CLI as a
...
means configurable-agent sees mo's output as opaque strings — no types.
...
pnpm workspace so configurable-agent always builds against local mo source.

Scope:

- Turn the repo into a pnpm workspace (root
  `pnpm-workspace.yaml` + root `package.json`). Only `mo` and
  `configurable-agent` join the workspace; gaia packages stay outside.
- Add `"@eggai-tech/mo": "workspace:*"` to configurable-agent's dependencies.
- Rewrite `configurable-agent/src/agent/tools/mo.ts` to call `runEvals({...})`
...
- Collapse the two Dockerfiles that handle mo:
  - `configurable-agent/Dockerfile` — switch build context to repo root, pnpm
    workspace install, `pnpm deploy` for a flat runtime tree.
  - `gaia/core-agent/Dockerfile` — drop the mo-build stage; the
    `/usr/local/bin/mo` wrapper points at configurable-agent's
